Spark, Python, LightGBM, FastAPI, Pandas, Cloud Storage
GitHub
• Built a scalable PySpark ETL pipeline for multi-GB NYC taxi data with data quality validation, schema
enforcement, and partitioned Parquet storage.
• Engineered time-series features (lag, rolling, calendar, rush-hour signals) for zone-level demand forecasting.
• Developed LightGBM models improving prediction accuracy over baseline methods.
• Designed recursive multi-step forecasting pipeline for future demand estimation.
• Implemented evaluation framework (MAE, RMSE, R2, SMAPE) with experiment tracking.
• Deployed FastAPI inference service with validated inputs and reusable feature pipelines.
• Built monitoring workflows including feature drift detection, residual analysis, and retraining triggers.# Project TODO & Findings
