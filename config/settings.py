...
PATH_PREV_TRAIN_DATA="C:/Users/nikhi/Downloads/Large-Scale-Taxi-Demand-Forecasting-System/features_store/selected_features.parquet"#data that was used to train the model latest
...
BEST_MODEL_PATH="" # to be filled by train.py after best model is chossen,shud change as models are changed during retraining/mlops
BEST_MODEL_NAME=""# to be filled by train.py after best model is chossen,shud change as models are changed during retraining/mlops
PRODUCTION_MODEL_INDEX=0#the version number/index of the model that is used in production
PROD_MODEL_PATH="" # path to the model beign used in prod
PROD_MODEL_NAME=""# Name of the model being used for prod
REGISTRY_PATH=Path("Large-Scale-Taxi-Demand-Forecasting-System/retrain_and_registry/registry.json")#PATH TO THE JSON FILE THAT STORES THE MODEL REGISTRY
REGISTRY_INDEX=1#INDEX/KEY OF THE LATEST ENTRY IN THE REGISTRY.JSON FILE DICT
MODEL_LIST=Path("C:/Users/nikhi/Downloads/Large-Scale-Taxi-Demand-Forecasting-System/models/artifacts/model_list.json")#file path to a json file that contains the name and path of the model with date trained and metadata for the model registry
BEST_MODEL_VER=1#VERSION OF THE BEST MODELS SAVED
